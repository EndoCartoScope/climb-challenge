...
std::string KeyFrame::getPoseColmapFormat()
{
    std::stringstream ss;
    ss << std::fixed << std::setprecision(15);
    // FORMAT: QW QX QY QZ TX TY TZ
    Sophus::SE3f pSE3 = GetPose();
    Eigen::Quaternionf q = pSE3.unit_quaternion();
    Eigen::Vector3f t = pSE3.translation();
    ss << q.w() << " " << q.x() << " " << q.y() << " " << q.z() << " " << t.x() << " " << t.y() << " " << t.z();
    return ss.str();
}

std::string KeyFrame::getPointsColmapFormat()
{
