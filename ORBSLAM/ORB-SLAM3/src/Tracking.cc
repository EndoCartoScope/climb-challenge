...
bool Tracking::ParseCamParamFile(cv::FileStorage &fSettings)
{
    ...
    if(sCameraName == "PinHole")
    {
        ...
        {
            std::cerr << "*Camera.width parameter doesn't exist or is not a int number*" << std::endl;
            b_miss_params = true;
        }
        ...
        {
            std::cerr << "*Camera.height parameter doesn't exist or is not a int number*" << std::endl;
            b_miss_params = true;
        }
        ...
        mpCamera->setImageSize(imageWidth, imageHeight);
...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    mCurrentFrame.mImageName = filename;
...
void Tracking::Track()
{
    ...
    if(mState==OK || mState==RECENTLY_LOST)
    {
        ...
        if(mCurrentFrame.isSet())
        {
            ...
            mlImageName.push_back(mCurrentFrame.mImageName);
            if(mCurrentFrame.mnId == mpReferenceKF->mnFrameId)
            {
                msFrameIdToKF.insert(mCurrentFrame.mnId);
            }
        }
        else
        {
