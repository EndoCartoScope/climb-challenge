...
void Map::AddKeyFrame(KeyFrame *pKF)
{
    ...
    if(mspKeyFrames.empty()){
        ...
        if(pKF->mpCamera)
        {
            mmCameraIDs[pKF->mpCamera->GetId()] = pKF->mpCamera;
        }
    }
    ...
    if(pKF->mpCamera)
    {
        unsigned int camID = pKF->mpCamera->GetId();
        if(mmCameraIDs.find(camID) == mmCameraIDs.end())
        {
            mmCameraIDs[camID] = pKF->mpCamera;
        }
    }
}
